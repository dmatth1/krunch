"""Bench: capture the WHOLE forward (12 layer-steps + final LN + head)
in ONE CUDA graph via torch.cuda.graph. One launch per token vs ~12+
launches per token in the per-layer-graph approach.
"""
...
def main():
    ...
    # Static buffers
    x_static = torch.zeros(1, T, n_embd, dtype=torch.float16, device=device)
    ...
    out_static = torch.zeros(T, model.args.vocab_size if hasattr(model.args, 'vocab_size') else head_w.shape[-1], dtype=torch.float16, device=device)
    ...
    # Bench unwrapped
    def fn_unwrapped():
        x_static.copy_(emb_w[tokens].view(1, T, n_embd))
        return full_forward()
    ...
    # Capture FULL forward in one graph
    print("capturing full-forward graph ...", flush=True)
    s = torch.cuda.Stream()
    ...
    # Reset state again before capture
    for i in range(N_LAYER):
        state_xx[i].zero_(); state_aa[i].zero_(); state_bb[i].zero_()
        state_pp[i].fill_(-1e30); state_ffn[i].zero_()
    ...
    with torch.cuda.graph(g):
        x_static.copy_(emb_w[tokens].view(1, T, n_embd))
        out_holder[0] = full_forward()
    print("graph captured", flush=True)

    def fn_graphed():
        g.replay()
    bench("cpp_full_graph_t1024", fn_graphed)

    # BlinkDL packed — DEFAULT (last-position logits only)
    state_b = None
    ...
    bench("blinkdl_packed_t1024_last_only", fn_blinkdl_last)

    # BlinkDL packed — full_output=True (matches what compress needs:
    # one logit vector per token to AC-encode each).
    def fn_blinkdl_full():
        nonlocal state_b
        state_b = None
        logits, state_b = model.forward(tok_list, state_b, full_output=True)
    bench("blinkdl_packed_t1024_full_output", fn_blinkdl_full)
